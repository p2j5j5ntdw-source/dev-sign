...
class SigningService: ObservableObject {
    ...
    func signIPA(
        ...
        await updateStatus("جاري تجهيز الملفات...", progress: 0.1)
        ...
        // إعادة الضغط
        await updateStatus("جاري إعادة ضغط الملف...", progress: 0.7)
        let outputURL = getOutputURL(name: newName)
        try FileManager.default.zipItem(at: payloadDir.deletingLastPathComponent(), to: outputURL)
    ...
    private func getOutputURL(name: String) -> URL {
        return FileManager.default.temporaryDirectory
            .appendingPathComponent("\(name)_unsigned.ipa")
    }
